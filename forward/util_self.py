...
def process_dataset(dataset, i, shape, padding, log_file, cval):
    # log_file.write('Cropping dataset {} ~~\n'.format(i))
    return crop_data(dataset[i], shape, padding, log_file, cval), i

def crop_dataset_threading(dataset, shape, log_file, padding, cval=0, num_threads=4):
    subsets = []
    sizes = []
    matches = []
    log_file.write('Cropping datasets ~~\n')
    # time = 0
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        # 创建一个将任务提交给executor的字典，关键是future，值是数据集的索引
        future_to_index = {executor.submit(process_dataset, dataset, i, shape, padding, log_file, cval): i for i in range(len(dataset))}
        for future in as_completed(future_to_index):
            result = future.result()
            subset, size, match = result[0]
            index = result[1]
            # 第一次时初始化subsets数组
            # if time == 0:
            #     subsets = subset
            ...
            # else:
                # subsets = np.concatenate((subsets, subset), axis=0)
            subsets.append(subset.astype(np.float32))
            sizes.append(size)
            matches.append(match)
            log_file.write('Finished cropping dataset {} ~~\n'.format(index))
