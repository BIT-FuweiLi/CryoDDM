...
# 检查输入参数
if [ "$#" -lt 2 ]; then
    echo "Usage: $0 <input1> <add_value>"
    exit 1
fi

# 读取输入参数,第一个是localtion 第二个是y大小
INPUT1=$1
ADD_VALUE=$2

# 运行 csparc2star.py 脚本
csparc2star.py "$INPUT1"  particles_relion.star


# 删除 cleaned_particles_relion.star 文件的前 13 行
sed -i '1,7d' particles_relion.star

# 运行 relion_star_handler 并修改 rlnCoordinateY 列
relion_star_handler --i particles_relion.star --o invert.star --operate rlnCoordinateY --multiply_by -1 --add_to "$ADD_VALUE"
